...
/*   Updated: 2022/08/02 18:41:58 by hbrent           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	final_camera(t_scene *scene)
{
	...
	{
		if (!(scene->camera.orient_vector[i] >= -1
				&& scene->camera.orient_vector[i] <= 1))
			return (INCORRECT);
		i++;
	}
	if (!(scene->camera.fov_in_degrees >= 0
			&& scene->camera.fov_in_degrees <= 180))
		return (INCORRECT);
